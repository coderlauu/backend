...
type StringToUnion<T extends string> = T extends `${infer S1}${infer Rest}` ? S1 | StringToUnion<Rest> : never

/** 字符串替换，类似js的字符串replace方法 */
/** eg: type result = Replace<'hello world', 'world', 'typescript'> 结果：'hello typescript' */
type Replace<
    Str extends string, 
    From extends string, 
    To extends string
 ...
 * -> 'a' | 'a.b' | 'a.b.c'
 */
// {}[keyof ObjectType & (string | number)] -> 只取它的 值
